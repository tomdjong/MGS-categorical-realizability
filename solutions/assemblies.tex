...
% \exercise{characterize-regular-epis}

% For the forward direction, assume that \(f \colon X \to Y\) coequalizes
% \(Z \overset{g}{\underset{h}{\rightrightarrows}} X\).
% By \cref{prop:coequalizers} there's an isomorphism of assemblies
% \[ s \colon Y \xrightarrow{\cong} X/\sim \, ,\]
% tracked by \(\pca s \in \AA\) and such that \(s(f(x)) = [x]\)
% for all \(x \in X\).

% Coequalizers are always epi, so \(f\) is epi and hence surjective by
% \cref{prop:characterize-epis-and-monos}.
% Suppose \(y \in Y\) and \(x \in f^{-1}(y)\).
% If \(\pca b \realizes_Y y\) then \(\pca{s} \pca{b} \realizes_{X/\sim} s(y) = [x]\),
% and so \(\pca{s} \pca{b} \realizes_X x^\prime\) for some \(x^\prime \sim x \in X\).
% Then we have that
% \[ f(x^\prime) = s^{-1}([x^\prime]) = s^{-1}([x]) = f(x) = y. \]

% For the converse, assume there exists \(\pca s \in \AA\) satisfying the stated property.
% Assuming the axiom of choice in \(\Set\), this gives us a section \(s \colon Y \to X\)
